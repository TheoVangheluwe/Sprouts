...
import AICanvas from '../components/game/pve/AICanvas';
        ...
        <div className="flex flex-col items-center justify-center h-full font-arcade text-white p-4">
            <div className="bg-gray-800 border-4 border-yellow-400 p-6 rounded-lg shadow-2xl w-full max-w-screen-lg text-center flex flex-row space-x-4">
                <div className="flex-1">
                    {phase === 'menu' && (
                        <>
                            <h1 className="text-3xl mb-4 animate-pulse text-yellow-300">🎮 Paramètres de la partie 🎮</h1>

                            <p className="mb-2">Nombre de points initiaux :</p>
                            ...
                            <div className="flex justify-center gap-4 mb-6">
                                {['easy', 'medium'].map(level => (
                                    <button
                                        key={level}
                                        className={`px-4 py-2 rounded border-2 border-white transition transform hover:scale-105 ${
                                            difficulty === level ? 'bg-yellow-400 text-black' : 'bg-gray-700'
                                        }`}
                                        onClick={() => setDifficulty(level)}
                                    >
                                        {level === 'easy' ? 'Facile' : 'Moyen'}
                                    </button>
                                ))}
                            </div>
                    ...
                    {phase === 'game' && (
                        <>
                            <h1 className="text-2xl font-bold mb-4 text-yellow-300">🎲 Joueur contre IA</h1>
                            ...
                            <div className="flex justify-center items-center w-full" style={{ height: '500px' }}>
                                <AICanvas
                                    ...
                                    addMove={addMove}
                                    difficulty={difficulty} // <- prop ajoutée ici
                                />
                            </div>
